# Hi, I'm Phanie

Full Stack Data and Machine Learning Engineer | Bioinformatics Graduate

**What I work with:**
- Data Analytics and Visualization
- Machine Learning and Deep Learning
- Bioinformatics and Computational Biology
- Web Applications with MERN, Next.js, Flask, and FastAPI

**Currently learning and exploring:**
- Geospatial Analysis
- 3D Visualization and Point Cloud Processing

Interested to talk more about these topics? Feel free to reach out!

---

## Portfolio

For a comprehensive showcase of my projects, case studies, and technical work, please visit:

### [Portfolio Guide Repository](https://github.com/lymphoidcell/Portfolio-Guide)

This repository contains organized collections of:
- Data analytics projects
- Machine learning models
- Bioinformatics analysis
- Web applications
- Case studies
- SQL challenges and more

---

## Highlighted Projects

| Date Started | Project | Description | Tech Stack | Status |
|--------------|---------|-------------|------------|--------|
| 24 October 2025 | Undisclosed | TBA | TBA | ON GOING |
| 16 November 2025 | Undisclosed | TBA | TBA | ON GOING |

---

## Collaboration

| Date Started | Title | About | Status | Category |
|--------------|-------|-------|--------|----------|
| 10 November 2025 | [Daily Math](https://github.com/lymphoidcell/daily-math) | A collaborative daily math problem-solving project. | ON GOING | Mathematics |

---

## Tech Stack

**Languages:** Python, SQL, R, JavaScript/TypeScript

**Data & ML:** Pandas, NumPy, Scikit-learn, TensorFlow, PyTorch, Biopython

**Bioinformatics:** Nextflow, Bioconductor, Linux/Unix scripting, Tidyverse

**Visualization:** Tableau, Power BI, Plotly

**Web Development:** MERN Stack, Next.js, FastAPI

**Tools:** Git, Databricks, Docker, Jupyter Notebooks, VS Code

---

## Contact

Feel free to reach out for collaborations or opportunities:

- Email: [phaniesql@gmail.com](mailto:phaniesql@gmail.com)
- LinkedIn: [linkedin.com/in/thscho](https://linkedin.com/in/thscho)

---

<div align="center">
  <sub>Open to collaborations and interesting projects</sub>
</div>
